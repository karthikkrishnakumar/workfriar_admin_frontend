...
const TimeSummary = ({ id }: TimeSummaryProps) => {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        ...
        const response = await fetchTimeLoggedByProjectId(id, sDate, eDate);
        if (response.status) {
  ...
  const formatDate = (date: string) => {
    ...
    const formattedDate = `${year}-${(month || "0").padStart(2, "0")}-${(
      day || "0"
    ).padStart(2, "0")}`;
  ...
  return (
    <div className={styles.tableWrapper}>
      <CustomTable columns={columns} data={filteredProjectTeam} />
    </div>
  );
};
