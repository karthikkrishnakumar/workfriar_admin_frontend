...
import styles from "./indian-holidays.module.scss";
...
const IndianHolidays: React.FC<AllHolidaysProps> = ({ year , refetch = false }) => {
  ...
  const fetchHolidays = async (year: string) => {
    ...
    const response = await UseHolidayServices().fetchIndianHolidays(yearInt);
