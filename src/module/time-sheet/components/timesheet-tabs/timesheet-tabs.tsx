...
const TimesheetsTabs = () => {
  ...
  useEffect(()=>{
    fetchTimesheets(setTimeSheetdata,startDate?.toISOString(),endDate?.toISOString());
    setLoadig(false);
    console.log("setLoad")
  },[startDate,endDate]);
  
  const tabs = [
    { key: "1", label: "All timesheets", content: <AllTimesheetsTable timesheetData={timeSheetData} setTimeSheetData={setTimeSheetdata}  /> },
  ...
  return (
    <div className={styles.timesheetTabsWrapper}>
      {loading ? (
        <div>
          <SkeletonLoader paragraph={{rows:2}} classNameItem={styles.customSkeleton}  />
          <SkeletonLoader count={1} paragraph={{rows:15}} classNameItem={styles.customSkeleton}/>
        </div>
      ) : (
        <div>
          <TabComponent
